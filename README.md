...
Creator AI now includes email/password authentication with JWT access tokens.
...
For local development, the backend defaults to SQLite if `DATABASE_URL` is not set. For production, use PostgreSQL.
...
Set this to the deployed FastAPI URL in Vercel.
