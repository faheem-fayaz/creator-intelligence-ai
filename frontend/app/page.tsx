...
export default function HomePage() {
  ...
  async function analyzeVideo() {
    try {
      setLoading(true);
      const response = await axios.get(`${API_URL}/video-score`, {
        params: { url },
      });
  ...
  async function analyzeHook() {
    try {
      setLoading(true);
      const response = await axios.get(`${API_URL}/hook-analysis`, {
        params: { url },
      });
    ...
    <main className="min-h-screen bg-black text-white flex">
      ...
      <section className="flex-1 p-10 overflow-y-auto">
        <div className="max-w-6xl mx-auto">
          ...
          {/* MAIN URL INPUT — hidden on compare tab */}
          {activeTab !== "compare" && (
              ...
              <div className="flex gap-4">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    if (activeTab === "score") analyzeVideo();
                    else if (activeTab === "hook") analyzeHook();
                    else if (activeTab === "chat") askQuestion();
                    else if (activeTab === "transcript") getTranscript();
                  }}
          ...
          {activeTab === "score" && analysis && (
            <>
              ...
              <div className="grid lg:grid-cols-3 gap-6">
                ...
                <AnalysisCard title="Improvements" items={analysis.improvements} />
              </div>
            </>
          )}
          ...
          {activeTab === "hook" && hookData && (
            <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-8 text-zinc-300 leading-8 prose prose-invert max-w-none">
              <ReactMarkdown>{hookData.analysis}</ReactMarkdown>
            </div>
          )}
          ...
          {activeTab === "chat" && (
            <div className="space-y-6">
              ...
              {chatData && (
                <>
                  <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-8 text-zinc-300 leading-8 prose prose-invert max-w-none">
                    <ReactMarkdown>{chatData.answer}</ReactMarkdown>
                  </div>
          ...
          {activeTab === "compare" && (
            <div className="space-y-6">
              ...
              <input
                value={question}
                onChange={(e) => setQuestion(e.target.value)}
                placeholder="Comparison question (or pick one below)"
                className="w-full bg-zinc-900 border border-zinc-700 rounded-2xl px-5 py-4 outline-none"
              />
              ...
              <div className="flex gap-3 flex-wrap">
                <button
                  onClick={() => setQuestion("Why did Video A get more engagement than Video B?")}
                  className="bg-zinc-800 hover:bg-zinc-700 px-4 py-2 rounded-xl text-sm"
                >
                  Engagement
                </button>
                <button
                  onClick={() => setQuestion("Compare the hooks in the first 5 seconds")}
                  className="bg-zinc-800 hover:bg-zinc-700 px-4 py-2 rounded-xl text-sm"
                >
                  Hooks
                </button>
                <button
                  onClick={() => setQuestion("Suggest improvements for Video B")}
                  className="bg-zinc-800 hover:bg-zinc-700 px-4 py-2 rounded-xl text-sm"
                >
                  Improvements
                </button>
              ...
              <button
                disabled={loading}
                onClick={() => {
                  if (!isValidYoutubeUrl(video1) || !isValidYoutubeUrl(video2)) {
                    toast.error("Please enter valid YouTube URLs for both videos");
                    return;
                  }
                  if (!question) {
                    toast.error("Please enter a comparison question");
                    return;
                  }
                  compareVideos();
                }}
                className="bg-gradient-to-r from-purple-500 to-pink-500 px-8 py-4 rounded-2xl font-semibold disabled:opacity-50 disabled:cursor-not-allowed w-full"
              >
                {loading ? (
              ...
              {compareData && (
                <div className="space-y-6">
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    ...
                    <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6">
                      ...
                      <div className="space-y-2 text-zinc-300 text-sm">
                        <MetaRow label="Creator" value={compareData.video_a_metadata?.creator || "Unknown"} />
                        <MetaRow label="Views" value={compareData.video_a_metadata?.views?.toLocaleString() || "0"} />
                        <MetaRow label="Likes" value={compareData.video_a_metadata?.likes?.toLocaleString() || "0"} />
                        <MetaRow label="Comments" value={compareData.video_a_metadata?.comments?.toLocaleString() || "0"} />
                        <MetaRow label="Followers" value={compareData.video_a_metadata?.followers?.toLocaleString() || "0"} />
                        <MetaRow label="Duration" value={`${compareData.video_a_metadata?.duration || 0}s`} />
                        <MetaRow label="Upload Date" value={compareData.video_a_metadata?.upload_date || "N/A"} />
                        <MetaRow label="Engagement Rate" value={`${compareData.video_a_metadata?.engagement_rate || 0}%`} />
                        <MetaRow label="Chunks Retrieved" value={compareData.video_1_sources?.length || 0} />
                      </div>
                    </div>
                    ...
                    <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6">
                      ...
                      <div className="space-y-2 text-zinc-300 text-sm">
                        <MetaRow label="Creator" value={compareData.video_b_metadata?.creator || "Unknown"} />
                        <MetaRow label="Views" value={compareData.video_b_metadata?.views?.toLocaleString() || "0"} />
                        <MetaRow label="Likes" value={compareData.video_b_metadata?.likes?.toLocaleString() || "0"} />
                        <MetaRow label="Comments" value={compareData.video_b_metadata?.comments?.toLocaleString() || "0"} />
                        <MetaRow label="Followers" value={compareData.video_b_metadata?.followers?.toLocaleString() || "0"} />
                        <MetaRow label="Duration" value={`${compareData.video_b_metadata?.duration || 0}s`} />
                        <MetaRow label="Upload Date" value={compareData.video_b_metadata?.upload_date || "N/A"} />
                        <MetaRow label="Engagement Rate" value={`${compareData.video_b_metadata?.engagement_rate || 0}%`} />
                        <MetaRow label="Chunks Retrieved" value={compareData.video_2_sources?.length || 0} />
                      </div>
                    </div>
                  </div>
                  ...
                  {/* AI COMPARISON */}
                  <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-8 text-zinc-300 leading-8 prose prose-invert max-w-none">
                    <ReactMarkdown>{compareData.comparison}</ReactMarkdown>
                  </div>
              ...
              <div className="space-y-5 max-h-[700px] overflow-y-auto">
                {transcriptData.segments?.map((segment: any, index: number) => (
                  <div key={index} className="border-b border-zinc-800 pb-4">
                    ...
                    <div className="text-zinc-300 leading-7">{segment.text}</div>
                  </div>
                ))}
              </div>
            </div>
...
function ScoreCard({ title, score }: any) {
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6">
      <div className="text-zinc-400 mb-3">{title}</div>
